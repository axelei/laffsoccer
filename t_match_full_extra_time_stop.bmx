...
Type t_match_full_extra_time_stop Extends t_match_mode
	
	Method New()
		
		Self.display_controlled_player = False
		match_status.timer = match_status.length * 120 / 90
	...
	Method update()
		...
		If (Self.frame_diff(frame, Self.frame0) > 3*SECOND)
			game_action_queue.push(AT_NEW_FOREGROUND, GM.MATCH_END_POSITIONS)
			Return
		EndIf
