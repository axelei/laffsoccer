...
class PngEditor {
    ...
    static ByteArrayInputStream swapPalette(InputStream in, InputStream paletteFile) throws IOException {
        ...
        // --- WRITE NEW PALETTE ---
        int paletteOffset = searchPaletteOffset(bytes);

        // read data size
        int dataSize = 0;
        for (int i = 0; i < 4; i++) {
            dataSize = 256 * dataSize + (bytes[paletteOffset + i] & 0xFF);
        }
        ...
        // write new crc
        int crc32 = getCrc(bytes, paletteOffset + 4, dataSize + 4);

        for (int i = 0; i < 4; i++) {
            bytes[paletteOffset + 8 + dataSize + i] = (byte) ((crc32 >> (8 * (3 - i))) & 0xFF);
        }
    ...
    static ByteArrayInputStream editPalette(InputStream in, List<RgbPair> rgbPairs) throws IOException {
        ...
        for (int i = 0; i < (dataSize / 3); i++) {
            ...
            int r = bytes[paletteOffset + 8 + 3 * i] & 0xFF;
            int g = bytes[paletteOffset + 8 + 3 * i + 1] & 0xFF;
            int b = bytes[paletteOffset + 8 + 3 * i + 2] & 0xFF;
